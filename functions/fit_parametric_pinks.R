fit_parametric_pinks <-
  function(this_stock,
    ...
    if (delta_returns) {
      ...
      ogret <- ogret[-c(1:2)] # account for lags below, fix this if you add more filters
    }
    ...
    if (run_wide) {
      ...
      stock_data <- stock_data %>%
        select(year, returns, training, juvenile_index) %>%
        left_join(buddies, by = "year")
      
      gam_formula <-
        as.formula(paste0(
          "returns ~ s(year) + s(juvenile_index) + ",
          paste(colnames(buddies %>% select(-year)), collapse = "+")
        ))
    
      } else {
      gam_formula <-
        as.formula("returns ~ s(year) +s(juvenile_index) + s(rl1) + s(rl2)")
    ...
    #   geom_line(aes(year, pred))
      ...
      juvenile_model <- lm(returns ~ juvenile_index, data = stock_data %>% filter(training))
      
      forecasts <- predict(juvenile_model, newdata = stock_data)
      
      stock_forecasts <- stock_data %>% 
        mutate(pred = forecasts, 
               delta_returns = FALSE, 
               run_wide = FALSE,
               model_type = model)
